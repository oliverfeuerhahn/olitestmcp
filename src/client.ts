...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['TEST_STAIN_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TEST_STAIN_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Test Stain API.
 */
export class TestStain {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['TEST_STAIN_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TEST_STAIN_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.TestStainError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the TestStain client with an apiKey option, like new TestStain({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? TestStain.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('TEST_STAIN_LOG'), "process.env['TEST_STAIN_LOG']", this) ??
      defaultLogLevel;
  ...
  static TestStain = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static TestStainError = Errors.TestStainError;
...
TestStain.PetResource = PetResource;
TestStain.Store = Store;
TestStain.UserResource = UserResource;
export declare namespace TestStain {
