...
import { APIPromise } from 'test-stain/core/api-promise';

import util from 'node:util';
import TestStain from 'test-stain';
import { APIUserAbortError } from 'test-stain';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['TEST_STAIN_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: TestStain) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new TestStain({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new TestStain({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new TestStain({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['TEST_STAIN_LOG'] = 'debug';
      const client = new TestStain({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['TEST_STAIN_LOG'] = 'not a log level';
      const client = new TestStain({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'TEST_STAIN_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['TEST_STAIN_LOG'] = 'debug';
      const client = new TestStain({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new TestStain({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['TEST_STAIN_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new TestStain({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['TEST_STAIN_BASE_URL'] = 'https://example.com/from_env';
      const client = new TestStain({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['TEST_STAIN_BASE_URL'] = ''; // empty
      const client = new TestStain({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });

    test('blank env variable', () => {
      process.env['TEST_STAIN_BASE_URL'] = '  '; // blank
      const client = new TestStain({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new TestStain({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new TestStain({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new TestStain({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new TestStain({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new TestStain({ apiKey: 'My API Key', fetch: testFetch });
